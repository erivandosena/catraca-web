	...
	public function telaIdentificacao(){
		...
		if(isset($_GET['usua_id']) && isset($_GET['novo_nivel'])){
			...
			if(!isset($_POST['certeza'])){
				...
				if($usuarioDao->alteraNivelDeAcesso($usuario)){
					$this->view->formMensagem("sucesso", "Nível Alterado com Sucesso!");					
				}else{
					$this->view->formMensagem("erro","Erro na tentativa de alterar nível.");
		...
		if(isset($_GET['numero_cartao'])){
			...
			if($idCartao){
				...
				$args = array($cartao, $usuario, $tipo, $strNivelAcesso, $imagem, $vinculo);				
				$this->view->formIdentificacao($args);
//				$this->view->listaBotoes($vinculo);
							
				if(!$vinculo->isActive()){
					$this->view->formMensagem("erro", "O vinculo não está ativo");
					...
					if(isset($_GET['cartao_renovar'])){
						if(isset($_POST['certeza'])){
							...
							{
								$this->view->formMensagem("err", "Esse usuário já possui vínculo válido.");								
