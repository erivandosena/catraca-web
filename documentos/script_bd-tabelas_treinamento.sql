...
-- Copyright    : Unilab                                                                                              --
-- Data_criacao : 10/05/2016                                                                                      --
-- Data_revisao : 10/05/2016                                                                                      --
-- Status       : TREINAMENTO                                                                                     --
----------------------------------------------------------------------------
...
CREATE DATABASE treinamento WITH OWNER = catraca ENCODING = 'UTF8';
COMMENT ON DATABASE treinamento IS 'Bando de dados de treinamento.';

-- Conectar a base treinamento
\connect treinamento;
...
(
  audi_id serial NOT NULL, -- Campo para chave primaria da tabela.
  audi_pagina character varying(200), -- Endereco da pagina utilizada pelo usuario.
  audi_data timestamp without time zone, -- Data/hora da atividade.
  usua_id integer NOT NULL, -- Campo para chave estrangeira da tabela usuario.
  audi_observacao character varying(200), -- Observacoes referente a atividade atual do usuario.
  CONSTRAINT pk_audi_id PRIMARY KEY (audi_id ), -- Chave primaria da tabela auditoria.
  CONSTRAINT fk_usua_id FOREIGN KEY (usua_id) REFERENCES usuario (usua_id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION -- Chave estrangeira da tabela usuario.
);
ALTER TABLE auditoria OWNER TO catraca; 
COMMENT ON TABLE auditoria IS 'Tabela que armazena as informacoes de usuario que operou o sistema.';
COMMENT ON COLUMN auditoria.audi_id IS 'Campo para chave primaria da tabela.';
COMMENT ON COLUMN auditoria.audi_pagina IS 'Endereco da pagina utilizada pelo usuario.';
COMMENT ON COLUMN auditoria.audi_data IS 'Data/hora da atividade.';
COMMENT ON COLUMN auditoria.usua_id IS 'Campo para chave estrangeira da tabela usuario.';
COMMENT ON COLUMN auditoria.audi_observacao IS 'Observacoes referente a atividade atual do usuario.';
COMMENT ON CONSTRAINT pk_audi_id ON auditoria IS 'Chave primaria da tabela auditoria.';
COMMENT ON CONSTRAINT fk_usua_id ON auditoria IS 'Chave estrangeira da tabela usuario.';
